...
while ((m = routeRegex.exec(appCode)) !== null) {
  routePatterns.push(m[1]);
}

console.log('Registered App.tsx Route Patterns:', routePatterns.length);

function matchesRoute(pathname) {
  const p = pathname.replace(/\/$/, '') || '/';
  for (const pattern of routePatterns) {
    if (pattern === p) return true;
    if (pattern.endsWith('/:slug*') || pattern.endsWith('/:division*')) {
      const prefix = pattern.replace(/\/:[^/]+\*$/, '');
      if (p === prefix || p.startsWith(prefix + '/')) return true;
    }
    if (pattern === '/:year/:month/:day/:slug' || pattern === '/:year/:month/:day/:slug*') {
...
userUrls.forEach(raw => {
  const u = new URL(raw.trim());
  const path = u.pathname.replace(/\/$/, '') || '/';
  if (path === '/wp-includes/wlwmanifest.xml' || path === '/naujienos/feed') {
    fullyResolved++;
  } else if (matchesRoute(path)) {
    fullyResolved++;
  } else {
...
console.log(`Audited ${userUrls.length} user URLs:`);
console.log(`Fully Resolved & Handled: ${fullyResolved} / ${userUrls.length} (${(fullyResolved/userUrls.length*100).toFixed(1)}%)`);
if (unresolved.length > 0) {
  console.log('Unresolved:', unresolved);
} else {
  console.log('SUCCESS: 100% OF USER REQUESTED URLS ARE DIRECTLY RESOLVED WITH CONTENT!');
}
